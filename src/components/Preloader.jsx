...
const Preloader = ({ onComplete }) => {
    ...
    const ctx = gsap.context(() => {
      ...
      paths.forEach(path => {
        ...
        gsap.set(path, {
          ...
          fill: "transparent",
          stroke: "#FFC800", // Clinical Yellow Vector Line
          strokeWidth: 2
        });
      });

      // 2. High-Speed Tracing
      tl.to(".wasp-shape", {
        strokeDashoffset: 0,
        duration: 1.8,
        ease: "power3.inOut",
      ...
      // Physical Slot Machine Rotation (Synchronized Snapping)
      const snapModifier = {
        y: function (y) {
          return (Math.trunc(parseFloat(y) / rowH) * rowH) + "em";
        }
      };

      tl.to(unitsRef.current, { y: `-${100 * rowH}em`, duration: 1.8, ease: "expo.inOut", modifiers: snapModifier }, 0);
      tl.to(tensRef.current, { y: `-${10 * rowH}em`, duration: 1.8, ease: "expo.inOut", modifiers: snapModifier }, 0);
      tl.to(hundRef.current, { y: `-${1 * rowH}em`, duration: 1.8, ease: "expo.inOut", modifiers: snapModifier }, 0);

      // 3. Ignition Flash (Hard Cut, No Blur)
      tl.to(".wasp-shape", {
        fill: "#FFC800",
        stroke: "transparent",
        duration: 0.1,
      ...
      // 4. Numbers disappear instantly when they get to 100
      tl.to(".preloader-counter", {
        opacity: 0,
        duration: 0.01 // Hard cut
      }, 1.8);

      // 5. Fast 360 Premium Spin (Snappy physical ease)
      // 5. Fast Premium Spin
      tl.to(waspRef.current, {
        rotation: 360,
        scale: 1.05, 
        duration: 0.6, // Rapid, crisp spin
        ease: "power3.inOut" 
      }, 1.7); 

      // Brief 0.2s pause after spin (1.7 + 0.6 + 0.2)
      tl.addLabel("transition", 2.5); 

      // TRIGGER HEADER ANIMATION START HERE
      tl.call(onComplete, null, "transition");

      // 6. Logo exits sharply
      tl.to(waspRef.current, {
        opacity: 0,
        y: -40, 
        scale: 0.9,
        duration: 0.4, // Fast, clean dissolve
        ease: "power2.inOut"
      }, "transition");

      // 6. Monolithic Curtain Whip (Fast, brutalist lift)
      tl.to(".curtain", {
        yPercent: -100,
        duration: 1.0, // Snappy 1.0s whip
        ease: "expo.inOut" 
      }, "transition");
  ...
  return (
    <div ref={overlayRef} className="fixed inset-0 z-[999] bg-transparent flex flex-col items-center justify-center pointer-events-none">

      {/* Monolithic Dark Curtain */}
      <div className="curtain absolute inset-0 bg-[#050505] z-0 pointer-events-auto" />

      {/* Wrapper for the UI */}
      <div className="absolute inset-0 w-full h-full z-10 flex items-center justify-center pointer-events-none">

        {/* Center: The Wasp Canvas */}
        <div className="relative flex flex-col items-center justify-center w-full h-full pointer-events-none">
          <div ref={waspRef} className="w-48 h-40 md:w-64 md:h-48 flex items-center justify-center">
            <svg viewBox="0 0 450 320" className="w-full h-full overflow-visible">
              <path
                className="wasp-shape"
                fillRule="evenodd"
                clipRule="evenodd"
                d={osaLogoPath}
              />
        ...
        {/* Bottom Right: Abstract Monospace Typography (Tight Layout) */}
        <div className="preloader-counter absolute bottom-12 right-12 flex items-baseline select-none">
          <div
            style={{ height: `${rowH}em` }}
            className="text-[14vw] md:text-[10vw] text-white/30 flex overflow-hidden tracking-tighter font-light leading-none font-mono"
          >
            {/* Hundreds Column */}
            <div ref={hundRef} className="flex flex-col text-center shrink-0">
              <div className="flex items-center justify-center shrink-0" style={{ height: `${rowH}em` }}>0</div>
              <div className="flex items-center justify-center shrink-0" style={{ height: `${rowH}em` }}>1</div>
            </div>

            {/* Tens Column */}
            <div ref={tensRef} className="flex flex-col text-center shrink-0">
              {[...Array(11)].map((_, i) => (
                <div key={`t-${i}`} className="flex items-center justify-center shrink-0" style={{ height: `${rowH}em` }}>{i % 10}</div>
              ))}
            </div>

            {/* Units Column */}
            <div ref={unitsRef} className="flex flex-col text-center shrink-0">
              {[...Array(101)].map((_, i) => (
                <div key={`u-${i}`} className="flex items-center justify-center shrink-0" style={{ height: `${rowH}em` }}>{i % 10}</div>
              ))}
            </div>
          </div>

          {/* Command Line Block Cursor */}
          <span className="text-[3vw] md:text-[2vw] text-[#FFC800] leading-none ml-3 mb-3 md:mb-4 self-end font-mono animate-pulse">
            █
          </span>
        </div>
