...
const Preloader = ({ onComplete }) => {
  ...
  useLayoutEffect(() => {
    // 6. THE EDITORIAL GRID TRANSITION
    const ctx = gsap.context(() => {
      ...
      // 1. Initial State
      gsap.set(".blind-col", { scaleY: 1, transformOrigin: "top" });
      ...
      paths.forEach(path => {
        ...
        gsap.set(path, { 
          ...
          fill: "transparent", 
          stroke: "#FFC800", // Matches O.S.A theme
          strokeWidth: 2 
        });
      });
      ...
      // 4. Physical Slot Machine Rotation (Snapping)
      const snapModifier = {
        y: function(y) {
      ...
      tl.to(unitsRef.current, { y: `-${100 * rowH}em`, duration: 3, ease: "power3.inOut", modifiers: snapModifier }, 0);
      tl.to(tensRef.current, { y: `-${10 * rowH}em`, duration: 3, ease: "power3.inOut", modifiers: snapModifier }, 0);
      tl.to(hundRef.current, { y: `-${1 * rowH}em`, duration: 3, ease: "power3.inOut", modifiers: snapModifier }, 0);
      ...
      // Final Punch-In for the Hero Content
      tl.fromTo(".hero-element", 
        { y: 80, opacity: 0 }, 
        { y: 0, opacity: 1, duration: 1.6, stagger: 0.1, ease: "expo.out" }, 
        "transition+=0.6"
      );
    });
  ...
  return (
    <div ref={overlayRef} className="fixed inset-0 z-[999] bg-transparent flex flex-col items-center justify-center pointer-events-none">
      ...
      {/* 11-Column Architectural Blinds */}
      <div className="absolute inset-0 flex z-0 pointer-events-auto">
         {[...Array(11)].map((_, i) => (
           <div key={`blind-${i}`} className="blind-col flex-1 bg-[#050505] h-full origin-top border-x border-white/[0.02]" />
         ))}
      </div>

      {/* Center: The Wasp Canvas & Status */}
      <div className="preloader-ui relative z-10 flex flex-col items-center justify-center w-full h-full mt-8 pointer-events-none">
        <div ref={waspRef} className="w-48 h-40 md:w-64 md:h-48 flex items-center justify-center drop-shadow-[0_0_30px_rgba(255,200,0,0.2)]">
        ...
        <span className="text-[#666666] text-[10px] md:text-[11px] font-mono uppercase tracking-[0.8em] mt-12 w-full text-center opacity-60">
          {percent < 30 ? 'INIT_SEQUENCE' : percent < 70 ? 'PROTOCOL_SYNC' : percent < 100 ? 'ALMOST_THERE' : 'SYSTEM_READY'}
        </span>
      </div>

      {/* Bottom Right: Monospace Percentage Data */}
      <div className="preloader-ui absolute bottom-12 right-12 flex items-baseline z-10 select-none">
        ...
        <span className="text-[4vw] md:text-[3vw] text-[#FFC800] leading-none ml-2 mb-2 self-end font-mono">
          %
        </span>
      </div>
